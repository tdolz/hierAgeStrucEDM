...
#grob plot of each age by each. 
agelist <-names(preylists)[3:22]
for (i in 2:length(agelist)){
  yrcls <-noquote(agelist[i])
  prev_yrcls <-noquote(agelist[i-1])
  
  p <-preylists%>%
  mutate(lag_prev_yrcls = lag(prev_yrcls,1))%>%
  filter(time_step >= 300 & time_step <=400)%>%
  ggplot(aes(yrcls,prev_yrcls, color=index))+
  geom_line()+
  xlab(yrcls)+ylab(paste(prev_yrcls, "-1"))+
    guides(color="none")+
  theme_classic()
  p
