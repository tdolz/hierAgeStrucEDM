...
We are importing the results of our simulations and manipulating the data for comprehensive visualizations here. 

importing from: 
OneSppGPloop_425_520_1010.R
...
**import the data**
```{r}
##one species BEVERTON-HOLT
#phi_1sppBH <-read.csv("OneSpeciesPhisBH.csv",header=T)%>%mutate(sim="1spp_BH")
#fits_1sppBH <-read.csv("OneSpeciesFitstatsBH.csv",header=T)%>%mutate(sim="1spp_BH")
#vars_1sppBH <-read.csv("varNage_1sppMEDBH.csv", header=T)%>%mutate(sim="1spp_BH")
##one species RICKER
#phi_1sppR <-read.csv("OneSpeciesPhisRICKER.csv",header=T)%>%mutate(sim="1spp_R")
#fits_1sppR <-read.csv("OneSpeciesFitstatsRICKER.csv",header=T)%>%mutate(sim="1spp_R")
#vars_1sppR <-read.csv("varNage_1sppRICKER.csv", header=T)%>%mutate(sim="1spp_R")
phi_1sppR <-read.csv("ONEsppPhisLN100.csv", header=T)%>%mutate(sim="I")
fits_1sppR <-read.csv("OneSppFitstatsLN100.csv", header=T)%>%mutate(sim="I")
##two species med
#phi_2spp <-read.csv("TwoSppMEDPhis.csv",header=T)%>%mutate(sim="2spp")
#fits_2spp <-read.csv("TwoSppMEDFitstats.csv",header=T)%>%mutate(sim="2spp")
#vars_2spp <-read.csv("varNage_2sppMED.csv",header=T)%>%mutate(sim="2spp")
phi_2spp <-read.csv("TwoSppMEDPhisLN100.csv", header=T)%>%mutate(sim="II")
fits_2spp <-read.csv("TwoSppMEDFitstatsLN100.csv", header=T)%>%mutate(sim="II")
##two species XL
#phi_2sppXL <-read.csv("TwoSppXLPhis.csv",header=T)%>%mutate(sim="2sppXL")
#fits_2sppXL <-read.csv("TwoSppXLFitstats.csv",header=T)%>%mutate(sim="2sppXL")
#vars_2sppXL <-read.csv("varNage_2sppXXL.csv",header=T)%>%mutate(sim="2sppXL")
phi_2sppXL <-read.csv("TwoSppXLPhisLN100.csv", header=T)%>%mutate(sim="III")
fits_2sppXL <-read.csv("TwoSppXLFitstatsLN100.csv", header=T)%>%mutate(sim="III")
```
...
phis <-bind_rows(phi_1sppR, phi_2spp, phi_2sppXL)%>%dplyr::select(-X)
...
ggplot(pivphimean, aes(rowname,model))+
  ...
  #geom_tile(aes(fill=phimean), show.legend = TRUE)+
...
**pivot**
```{r}
newfits <-bind_rows(fits_1sppR,fits_2spp,fits_2sppXL)%>%dplyr::select(-X)
...
**Boxplot1: Figure total abundance single index to total abundance summed estimates from hierarchical models**
under "approach", "sumhier" is the sum total abundance of the hierarchcial models. 

```{r}
## FITSTATS BOXPLOTS ##
fig2filt <-filter(nfitspiv,stat=="OOS_R2", approach %in% c("TAindex","sumhier"), model !="preyNT")%>%mutate(tslength=as.factor(tslength))
...
fig2filt%>%
 ...
 #scale_fill_manual(values = apcols)+
 ylab("out of sample r-squared")+
 #ylab("mean dynamic correlation (rho)")+
 facet_wrap(~sim, nrow=3, scales="free")+
...
**Boxplot1: but as a facet plot**
Boxplot of age structured model next to Ntotal model. 
```{r}
simcols <-c("prey205"="#c3a770","prey1010"="#96aad0","prey520"="#a97f8f","prey425"="#0477c2", "NT_diffyears"="#a74b22")


fig2filt%>%
  arrange(tslength)%>%
  ggplot(aes(tslength, value, fill=model))+
  geom_boxplot(aes(fill=model), alpha=0.7)+ 
 scale_fill_manual(values = simcols)+
 ylab("out of sample r-squared")+xlab("analysis")+
 facet_grid(sim~factor(tslength, levels=c('5','10','20','25',"100")), scales="free")+
 guides(fill=guide_legend(title="analysis"))+
 theme(
  axis.text.x = element_text(angle=90),
  strip.background =element_rect(fill="transparent"),
...
#ggsave("ntotallengths.png", path="/Users/tdolan/documents/postdoc/age structure/agestructfigs")
#dev.off()
```
**Boxplot1: but one at a time**
Boxplot of age structured model next to Ntotal model. 
```{r}
mods1 <-c("NT_diffyears", "prey1010","prey205","prey425","prey520")
...
for (i in 1:length(tslength)){
    ...
    filter(sim==simlist[j])
    #filter(sim=="III")
  ts <-tslength[i]
 p<- fits%>%arrange(tslength)%>%
 filter(stat %in% c("OOS_R2"))%>%
...
grid.arrange(arrangeGrob(mainplot, left = y.grob, bottom = x.grob,padding = unit(0, "line")))
...
fig2filt%>%
  ...
  geom_point(aes(y = value, color =approach), 
   ...
   #geom_violin()+
 #scale_fill_manual(values = hiercols)+
  #scale_color_manual(values = hiercols)+
...
#ggsave("ntotallengthsTanya2.png", width=12, height=5, dpi=300, path="/Users/tdolan/documents/postdoc/age structure/agestructfigs")
#dev.off()
...
```{r}
fig3filt <-filter(nfitspiv,stat=="OOS_R2", approach %in% c("TAindex","hier"), model %in% c("preyNT","prey205","prey1010","prey520","prey425"))%>%
...
fig3filt%>%
  ...
  geom_point(aes(y = value), color="grey",
               position = position_jitter(width=0.2), size = 1, alpha = 0.4) +
    geom_boxplot(width = 0.3, outlier.shape = NA, alpha = 0.8,fill="grey") +
    #geom_flat_violin(position = position_nudge(x = 0.2, y = 0), alpha = 0.8) +
...
**all the ages**
This is dumb because why would we be interested in 100 years of data of the hierarchical modls.YOu would want to compare equivalent length time series, you dumb dumb... 
```{r}

nfitspiv$age_class <-fct_relevel(nfitspiv$age_class, "all","V1","V2","V3","V4","V5","V6","V7","V8","V9","V10","V11","V12","V13","V14","V15","V16","V17","V18","V19","V20")

nfitspiv%>%
 filter(stat %in% c("OOS_R2"))%>%
  filter(sim=="III")%>%
  #filter(tslength != "100")%>%
  filter(model %in% c("prey205","prey1010", "prey520","prey425"))%>%
  ggplot(aes(age_class, value, fill=models))+
  geom_boxplot(aes(fill=models), alpha=0.7, notch=T, notchwidth = 0.8)+ 
 scale_fill_manual(values = simcols3)+
 ylab("out of sample r-squared")+
 #facet_grid(sim~model, scales="free")+
...
#ggsave("hiermodelsbetter.png", path="/Users/tdolan/documents/postdoc/age structure/agestructfigs")
#dev.off()
```
...
```{r}
nfitspiv$model2 <-fct_relevel(nfitspiv$model2, "5 years 20 ages",  "10 years 10 ages", "20 years 5 ages",  "25 years 4 ages")


sumfits <-nfitspiv %>% 
  filter(stat=="rho")%>%
  filter(model != "preyNT")%>% filter(tslength != "100")%>%
  group_by(sim, model2)%>% summarize('mean dynamic rho'=mean(value), "std. dev. dynamic rho"=sd(value), .groups="drop")%>%dplyr::rename(model=model2)%>%mutate(across(c(3,4), round,3))
