...
pxfit1 <-read.csv("tslength_ageclass_outputs/crossfits_sim1.csv",header=TRUE)
pxfit2 <-read.csv("tslength_ageclass_outputs/crossfits_sim2.csv",header=TRUE)
pxfit3 <-read.csv("tslength_ageclass_outputs/crossfits_sim3.csv",header=TRUE)
...
rhomat <-rhomat %>% #remove_rownames %>%
  mutate(time_step =time_step - test_length) 
rhomat <-rhomat[1:26,]
rhomat <-rhomat %>%  column_to_rownames(var="time_step") #when using the 10 year sequential
...
fig2 <-plot_ly(type="contour",
               ...
               #colorscale = list(seq(0.78,1.0,0.02),c("#dc2f02","#d00000","#9d0208","#6a040f","#370617","#03071e")),
               #coloraxis="coloraxis",
               colorbar=list(tickfont=list(size=16)),
               reversescale =F,
               autocontour = F, 
               #contours = list(start = 0, end = 1.0,size = 0.2), #adjust contours
               x=~rownames(rhomat2), y=~colnames(rhomat2), z=~t(rhomat2))%>%
      colorbar(title=metname, size=16)%>%
...
fig2 <-plot_ly(type="contour",
               colorscale='Jet',
               autocontour = F, 
               contours = list(start = 0, end = 1.0,size = 0.05), #adjust contours
               x=~5:15, y=~1:20, z=~tr2)%>%
      colorbar(title=metname, size=16)%>%
      layout(xaxis = list(title = 'years included',
                          tickfont = list(size = 16), titlefont = list(size = 16),
                          nticks=10),
                                     yaxis = list(title = 'number of age classes included',
                                                  tickfont = list(size = 16),titlefont = list(size = 16)),
                                     zaxis = list(title = 'r-squared',nticks=10, range=c(0,1)))
                                    # zaxis = list(title = metname))   
